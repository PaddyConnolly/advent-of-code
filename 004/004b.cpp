...
int process(std::string & input) {
  ...
  std::string line;
  std::vector<std::string> masterCharGrid;
  

  while(std::getline(stream, line)) {
    if (!line.empty()) {
      masterCharGrid.push_back(line);
    }
  }

  int rows = masterCharGrid.size();
  int cols = masterCharGrid[0].size();

  int total = 0;
  while (true) {
    std::vector<std::vector<int>> counts(rows, std::vector<int>(cols, 0));
    std::vector<std::string> charGrid = masterCharGrid;
